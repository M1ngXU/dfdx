		...
		#[cfg(not(feature = "ci-check"))]
		{
			...
			let max_nvcc_code = {
				let out = std::process::Command::new("nvcc")
					...
					.output()
					.expect(
						"`nvcc` failed. Ensure that you have CUDA installed and that `nvcc` is in \
						 your PATH.",
					);
				...
				if !codes.contains(&compute_cap) {
					panic!(
						"nvcc cannot target gpu arch {compute_cap}. Available nvcc targets are \
						 {codes:?}."
					);
				}
				*codes.last().unwrap()
			...
			if compute_cap > max_nvcc_code {
				println!(
					"cargo:warning=Lowering gpu arch {compute_cap} to max nvcc target \
					 {max_nvcc_code}."
				);
			...
			let children = kernel_paths
				...
				.map(|p| {
					std::process::Command::new("nvcc")
						...
						.spawn()
						.expect(
							"nvcc failed to start. Ensure that you have CUDA installed and that \
			...
			for (kernel_path, child) in kernel_paths.iter().zip(children.into_iter()) {
				let output = child.wait_with_output().expect(
					"nvcc failed to run. Ensure that you have CUDA installed and that `nvcc` is \
					 in your PATH.",
				);
				assert!(
